...
sub _copy_nvram_vmware ($self, $name, $vmware_openqa_datastore, $vmware_disk_path) {
    ...
    my $cmd =
      "set -x; nvram=\$(grep nvram $vmware_vmx_path | awk -F\\\" \'{print \$(NF-1)}\');" .
      "if [ -e $vmware_openqa_datastore" . '$nvram ]; then ' .
      "cp $vmware_openqa_datastore" . '$nvram ' . "${vmware_openqa_datastore}${name}.nvram;" .
      'fi;';
    my $retval = $self->run_cmd($cmd, domain => 'sshVMwareServer');
...
sub _copy_image_to_vm_host ($self, $args, $vmware_openqa_datastore, $file, $name, $basedir, $cdrom) {
    ...
    if ($cdrom || $backingfile) {
        if ($self->vmm_family eq 'vmware') {
            ...
            $self->_copy_nvram_vmware($name, $vmware_openqa_datastore, $vmware_disk_path);
        }
        else {
